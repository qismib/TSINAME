...
def autolabel(rects):
    ...
    for rect in rects:
        ...
        ax.text(rect.get_x() + rect.get_width()/2., height + 1, ('%f' % round(height,2)).rstrip("0") + " %", ha='center', va='bottom', fontsize=14)	
...
data_file = open("bb84_yorktown_scenario1_merged5000.txt", "r")
...
plt.suptitle("Backend: ibmq_5_yorktown - Scenario 1", fontsize=16, fontweight="bold")
plt.title("Probability of obtaining couples of identical correlated bits\nor different correlated bits", fontsize=14)

ax.set_ylabel("Probability (%)", fontsize=16)
rectangle2 = ax.bar([0,1], height=[prob_identical*100, prob_different*100], tick_label=["Identical", "Different"], color=["b", "r"]) 
plt.axis([-0.5, 2.4, 0, 110])cc
autolabel(rectangle2)

# Drawing the prediction's lines
x1 = [-0.5,2.5]
y1 = [25,25]
x2 = [-0.5,2.5]
y2 = [14.14,14.14]
x3_right = [-0.5,2.5]	# In order to avoid superpositioning with the text, I broke the line in 2 pieces
x3_left = [1.35, 2.5]
y3 = [39.14,39.14]
plt.plot(x1, y1, "c--", label="Theory", linewidth=2)
plt.text(1.5, 26, "Theory: 25%")
plt.plot(x2, y2, "y--", label="Expected noise", linewidth=2)
plt.text(1.5, 15.14, "Expected noise: 14.14%")
plt.plot(x3_right, y3, "g--", label="Theory + noise", linewidth=2)
#plt.plot(x3_left, y3, "g--", linewidth=2)
plt.text(1.5, 40.14, "Theory + noise: 39.14%")
plt.legend()
